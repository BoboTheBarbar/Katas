package de.katas.codewars.evaluateMathematicalExpression;
...
public class MathEvaluator {
    ...
    private double executeOperation(int index, DoubleBinaryOperator operation) {
        Integer indexRight = extractNumberIndex(index, x -> x + 1);
        Integer indexLeft = extractNumberIndex(index, x -> x - 1);

        String numberAsStringLeft = term.substring(indexLeft, index);
        String numberAsStringRight = term.substring(index + 1, indexRight + 1);

        double leftNumber = Double.parseDouble(numberAsStringLeft);
        double rightNumber = Double.parseDouble(numberAsStringRight);

        double result = operation.applyAsDouble(leftNumber, rightNumber);
        term.replace(indexLeft, indexRight + 1, Double.toString(result));
